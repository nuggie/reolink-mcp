...
Tool registration is deferred to the bottom of this module, after `mcp` is
constructed, via `reolink_mcp.tools.register_all(mcp)` — see
...
async def app_lifespan(server: FastMCP) -> AsyncIterator[AppContext]:
    """Startup: config-only validation, no camera I/O (D-06) —
    ...
    caught here. Shutdown: guaranteed, exception-tolerant logout of every
    connected camera (Pitfall 5, PITFALLS.md)."""
